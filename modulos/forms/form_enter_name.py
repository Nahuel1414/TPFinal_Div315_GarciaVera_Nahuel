...
def init_form_enter_name(dict_form_data: dict, jugador: dict):
    ...
    form['btn_confirm_name'] = Button(
        ...
        screen= form.get('screen'),
        font_path= var.FUENTE_ANCIENT,
        on_click= click_confirm_name,
        on_click_param= form
    )

    form['widgets_list'] = [
        form.get('title'), form.get('title_2'), form.get('subtitle'), form.get('subtitle_score'), form.get('btn_confirm_name')
    ]
...
def click_confirm_name(form_dict: dict):
    ...
    jugador_mod.set_nombre(form_dict.get('jugador'), form_dict.get('writing_text').text)


    ranking_actual = aux.cargar_ranking()
    jugador = form_dict.get('jugador')
    nombre = jugador.get('nombre', 'Player')
    puntaje = int(jugador.get('puntaje_actual', 0))

    nuevo_jugador = [nombre, puntaje]
    ranking_actual.append(nuevo_jugador)


    ranking_actual.sort(key=lambda x: x[1], reverse=True)
    aux.guardar_ranking(ranking_actual)
